...
| amount | decimal | 订单金额 |
| pay_status | int | 支付状态（1=已支付） |
| trade_no | string | 支付宝订单号 |
| paid_at | string | 支付时间（格式：Y-m-d H:i:s） |
| timestamp | int | 时间戳 |
| sign | string | 签名 |
...
签名前的字符串格式（按参数名ASCII码升序排列，最后加上`key=api_secret`）：

```
amount=1&merchant_order_no=M1763267881&paid_at=2025-11-16 12:39:00&pay_status=1&platform_order_no=BY220251116123810EA78A2EC&timestamp=1763267940&trade_no=2025111622001454171415453410&key=b618e5f95fb4ea279378582577459a172e6a457eda3c4625f71f863552aeb753
```

签名生成步骤：
1. 移除`sign`参数
2. 按键名ASCII码升序排序
3. 按照`key=value&`格式拼接（空值和null值跳过）
4. 在末尾添加`key=api_secret`
5. 对拼接后的字符串进行MD5加密并转大写
...
1. 验证签名
2. 处理业务逻辑
3. 返回字符串`success`（**不区分大小写**，可以是"success"、"SUCCESS"、"Success"等）

如果未返回`success`（转大写后为"SUCCESS"），系统会重复发送通知（最多尝试5次）。
